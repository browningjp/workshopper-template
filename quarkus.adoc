:hide-uri-scheme:
== Introduction to Quarkus on OpenShift [INNOVATION]
...
To get started, access the Che instance and log in using the username and password you’ve been assigned.

You will be prompted to update your account information before proceeding, enter the following in the respective fields:

[source]
----
Username: {{USER_ID}}
Email: {{USER_ID}}@no-reply.com
First Name: {{USER_ID}}
Last Name: {{USER_ID}}
----

Once you log in, you will be placed on the Get Started page. It gives you a view of the different projects that you can create create or pre-configure.

You will now create a Quarkus workspace for you to use. Select the Quarkus tile, then click Create a Custom Workspace on the top right-hand of the page.

Ensure that you are on Ready-To-Go-Stack tab.
...
In the Select Stack field, select the  `Quarkus Tools` option.

In the Projects field, remove the default `getting-started` project.

Next select the `git` tab and enter the following URL in the `git URL` field:

[source]
----
https://github.com/akouao/quarkus-lab
----

Click `add` to save your imported GitHub project selection.

image::quarkus-1.png[Imported GitHub project]

Scroll to the top of the page, and select the Import Devfile tab. Ensure that the URL option is selected.

In the URL field, enter the following devfile URL
...
Navigate to `src` → `main` → `java` → `org.acme.rest` in the project tree and double click on `GreetingResource.java`:
...
You can also click on the URL link labelled `web-development` on the right to open the same default page in a separate browser tab.
...
Here, we can see that the error states `"return hola"` is not a statement.
...
Open the `GreetingResource.java` file (in `src/main/java/org/acme/rest`) and return `return "hola"`; back to to `return "hello"`; in the editor.
...
Let’s modify the application and add a companion bean. In CodeReady, navigate to the `org.acme.getting-started.service.GreetingService.java` class in the project browser.
...
[source]
----
/projects/quarkus-lab/getting-started/target/getting-started-1.0.0-SNAPSHOT-runner -Dquarkus.http.port=8081
----
...
Finally, lets tidy up the cluster now that your introduction to Quarkus is complete. Please use the command

[source]
----
oc delete project quarkus-workshop-{{USER_ID}}
----
